...
export const getPassword = (...args: any[]) =>
...
export const genDocument = (digits: number) => {
  let document = "";
...
export const genMobilPhone = (countryCode: string) => {
  return `+${countryCode} ${genDocument(3)} ${genDocument(3)} ${genDocument(
    4
  )}`;
};

export const fillNumber = (num: string, max?: number) => {
